...
class DQN(object):
    ...
    def train(
        ...
        if os.path.exists(self.model_dir):
            ...
            target_model.load_weights(filepath=os.path.join(self.model_dir, "target_model"), )
                ...
                if memory.size > min_memory:
                    for _ in range(train_cycles):
                        ...
                        if target_model_update >= 1 and tf.equal(optimizer.iterations % target_model_update, 0):
                            eager_utils.update_variables(target_variables, model_variables)
                        else:
                            eager_utils.update_variables_soft(target_variables, model_variables, target_model_update)
