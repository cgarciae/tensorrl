...
def update_variables(target_variables, model_variables):

    updates = [target.assign(current) for target, current in zip(target_variables, model_variables)]

    return tf.group(*updates)


def update_variables_soft(target_variables, model_variables, beta):

    updates = [target.assign_add(beta * (current - target)) for target, current in zip(target_variables, model_variables)]
...
class DQN(object):
        ...
        with tf.Graph().as_default() as graph:
            ...
            with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
                train_op = optimizer.minimize(
                    ...
                    var_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=model_scope.name),
                )
            ...
            if target_model_update >= 1:
                update_target_op = tf.cond(
                    ...
                    lambda: update_variables(target_variables, model_variables),
                    lambda: tf.no_op(),
                )
            else:
                update_target_op = update_variables_soft(target_variables, model_variables, target_model_update)
            ...
            episode_summaries = tf.summary.merge([episode_length_summary, episode_reward_summary])
            ...
            while step < max_steps:
                ...
                for _ in range(train_cycles):
                    ...
                    if memory.nb_entries > min_memory:
                        ...
                        train_fetches = dict(train_op=final_train_op, )
    ...
    def eval(
        ...
        with tf.Graph().as_default() as graph:
            ...
            writer = tf.summary.FileWriter(os.path.join(self.model_dir, "eval"))
        ...
        with graph.as_default(), tf.Session(graph=graph) as sess:
            ...
            for step in range(max_steps):
                ...
                if terminal:
                    ...
                    ep_fetches = dict(episode_summaries=episode_summaries, )
